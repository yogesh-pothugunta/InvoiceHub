...
const createInvoice = async (req, res) => {
  try {
    const { clientId, clientSnapshot, items, dueDate, taxRate, discount, notes, terms, currency, status, isRecurring, recurringInterval } = req.body;
    ...
    const subtotal = parseFloat(processedItems.reduce((sum, i) => sum + i.amount, 0).toFixed(2));
    const taxAmount = parseFloat((subtotal * (taxRate || 0) / 100).toFixed(2));
    const discountAmt = parseFloat((discount || 0));
    const total = parseFloat((subtotal + taxAmount - discountAmt).toFixed(2));
    ...
    await User.findByIdAndUpdate(req.user.id, { $inc: { invoiceCounter: 1 } });

    // Update client stats if client linked
    if (clientId) {
...
const updateInvoice = async (req, res) => {
  try {
    ...
    if (items) {
      const processedItems = items.map(item => ({ ...item, amount: item.quantity * item.rate }));
      const subtotal = processedItems.reduce((sum, i) => sum + i.amount, 0);
      const taxAmount = subtotal * (taxRate || invoice.taxRate) / 100;
      const discountAmt = discount || invoice.discount || 0;
      req.body.subtotal = parseFloat(subtotal.toFixed(2));
      req.body.taxAmount = parseFloat(taxAmount.toFixed(2));
      req.body.total = parseFloat((subtotal + taxAmount - discountAmt).toFixed(2));
  ...
  try {
    ...
    if (status === 'paid') {
      ...
      if (invoice && invoice.client) {
        await Client.findByIdAndUpdate(invoice.client, { $inc: { totalPaid: invoice.total } });
      }
    }
    ...
    res.json({ success: true, message: `Status updated to ${status}`, data: invoice });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
const sendEmail = async (req, res) => {
  try {
    ...
    const pdfBuffer = await generateInvoicePDF(invoice, user);
    await sendInvoiceEmail(invoice, user, pdfBuffer, customMessage);
    ...
    res.json({ success: true, message: `Invoice emailed to ${invoice.clientSnapshot.email}` });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Email failed: ' + error.message });
  }
};
...
const exportCSV = async (req, res) => {
  try {
    ...
    const headers = ['Invoice #', 'Client', 'Email', 'Issue Date', 'Due Date', 'Subtotal', 'Tax', 'Total', 'Status'];
...
module.exports = { getInvoices, getInvoice, createInvoice, updateInvoice, deleteInvoice, updateStatus, downloadPDF, sendEmail, exportCSV };
