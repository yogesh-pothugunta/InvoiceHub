...
const createTransporter = () => {
  return nodemailer.createTransport({
    ...
    secure: false, // SendGrid port 587 ki false
    auth: {
...
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f5f5f5;">
  ...
  <div style="max-width:600px;margin:30px auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px;">
      ...
      <p style="color:#333;font-size:15px;">
        Dear <strong>${invoice.clientSnapshot.name}</strong>,
      </p>

      <div style="background:#E6F1FB;border-left:4px solid #185FA5;padding:16px 20px;border-radius:4px;margin-bottom:24px;">

        <p style="margin:0;color:#185FA5;">
          <strong>Invoice #${invoice.invoiceNumber}</strong>
        </p>

        <p style="margin:4px 0 0;color:#555;font-size:24px;font-weight:700;">
      ...
      <p style="color:#555;font-size:14px;line-height:1.7;white-space:pre-line;">
  ...
  const mailOptions = {
    from:
      ...
      `${companyName} <${process.env.EMAIL_USER}>`,
